  ...
  return (
    ...
    >
      ...
      >
        <img
          src={logo}
          className={`mr-3 h-6 sm:h-9 block float-left duration-500 ${
            open && "rotate-[360deg]"
          }`}
          alt="EduVibe Logo"
        />
        <p className={`duration-300 ${!open && "scale-0"}`}>
      ...
      <div>
        {coursesList.map((course, index) => (
          <Link
            ...
            key={course.id}
            index={index}
            className={`normal-case text-xl block items-center font-bold my-5 duration-300 ${
              !open && "scale-0"
            }`}
          >
